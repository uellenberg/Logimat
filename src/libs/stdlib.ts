...
inline function array_set(arr, idx, new_value) {
    // idx == idx is used to ensure that undefined isn't passed to range.
    range(1, min(10000, max(arr.length, (idx == idx ? idx : 0)))).map(v => v == idx ? new_value : (arr.length < v ? 0 : arr[v]))
}`;
